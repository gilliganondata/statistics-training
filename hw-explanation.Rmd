...
view_id <- Sys.getenv("GA_VIEW_ID_TRAINING_HW")
...
# For the sake of the example, let's fudge the data a little bit.
ga_data[ga_data$time_period == as.Date("2017-10-04"), 2] <- 7050
ga_data[ga_data$time_period == as.Date("2017-10-30"), 2] <- 6750
ga_data[ga_data$time_period == as.Date("2017-10-31"), 2] <- 6850
ga_data[ga_data$time_period == as.Date("2017-11-01"), 2] <- 6650
ga_data[ga_data$time_period == as.Date("2017-11-02"), 2] <- 6450
...
ga_plot_weekly <- ggplot(ga_data_weekly, mapping = aes(x = week, y = sessions)) +
  geom_line(color = "#00A2B1", size = 1) +
  geom_point(aes(y=sessions), size=3, shape = 19, colour = "#00A2B1") +
  ...
  scale_y_continuous(label=comma, expand = c(0, 0), limits = c(-1000, max(ga_data_weekly$sessions + 1000))) +
  theme_hw
...
ga_plot <- ggplot(ga_data_plot, mapping = aes(x = time_period)) +
  geom_line(aes(y = ga_data_plot$metric_all), color = "#00A2B1", size = 0.75) +
...
ga_plot <- ggplot(ga_data_plot, mapping = aes(x = time_period)) +
  ...
  geom_line(aes(y = ga_data_plot$metric_assess), color = "#00A2B1", size = 0.75) +
  geom_vline(aes(xintercept = cutoff_time_period), 
             color = "gray40", linetype = "dashed", size = 1) +
  scale_y_continuous(label=comma, expand = c(0, 0), limits = c(-1000, max_y)) +
  theme_hw
...
ga_plot <- ggplot(ga_data_plot, mapping = aes(x = time_period)) +
  geom_line(aes(y = ga_data_plot$metric_training), color = "#00A2B1", size = 0.75) +
...
ga_plot <- ggplot(ga_decomp_df_plot, mapping = aes(x = time_period, y = value, colour = key)) +
  ...
  scale_color_manual(values=c("#00A2B1", "#999999", "#999999", "#999999")) +
  theme_sparklines
