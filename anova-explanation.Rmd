...
## Histogram and Density Plot for Organic Search

```{r histograms_os, message=FALSE, echo=FALSE}

ga_organic_search_day <- ga_channel_day %>%
  filter(channelGrouping == "Organic Search")

# Organic Search histogram
gg_hist_organicsearch <- ggplot(ga_organic_search_day, mapping = aes(x = sessions)) +
  ...
  labs(y = "# of Observations", x ="Organic Search Sessions") +
  theme_histogram

gg_hist_organicsearch

# Organic Search density plot

# We're going to want to scale ylim just a smidge, so have to calculate the
# density on its own first
ps_density_os <- density(ga_organic_search_day$sessions)

# Now...make the plot
gg_density_organicsearch <- ggplot(ga_organic_search_day, mapping = aes(x = sessions)) +
  ...
  labs(x ="Organic Search Sessions") +
  theme_densityplot 

gg_density_organicsearch
...
```{r histograms-horizontal, message=FALSE, echo=FALSE}
...
gg_density_with_means <- gg_density_all +
  geom_vline(data = density_means, mapping = aes(xintercept = mean),
             linetype = "1121", size=1.5, colour="gray30")
...
##########
# Make a function to do a boxplot -- we're going to do several

make_boxplot <- function(df){
  
  ggplot(df, aes(x = channelGrouping, y = sessions)) +
  stat_boxplot(geom ='errorbar', width = 0.5, color = "gray50") +
  ...
  geom_boxplot(aes(ymin=..lower.., ymax=..upper..), 
               ...
               outlier.size = 1) + 
  xlab("(Daily) Sessions") +
  scale_y_continuous(label = comma, expand=c(0,0), limits=c(0,max(df$sessions) * 1.04)) +
  coord_flip() +
  theme_bar +
  theme(panel.grid.major.x = element_line(colour="gray80"),
        axis.text.x = element_text(size = 14, face="plain", color="gray40"),
...
boxplot <- make_boxplot(ga_channel_day %>% 
                          mutate(channelGrouping = factor(channelGrouping,
                                                          levels = rev(ga_channel_total$channelGrouping))))
boxplot

```

Run a 1-way ANOVA, perform a post hoc Tukey analysis, and then visualize which channels are related to each other.
...
channels_posthoc_df$channel_1 = factor(channels_posthoc_df$channel_1,
                                       levels = c("Social", "Direct",
                                                  "Paid\nSearch", "Referral",
                                                  "Organic\nSearch"))

channels_posthoc_df$channel_2 = factor(channels_posthoc_df$channel_2,
                                       levels = c("Display", "Organic\nSearch",
                                                  "Referral","Paid\nSearch",
                                                  "Direct"))


# Spread it out and see what happens
# foo <- spread(channels_posthoc_df, channel_2, `p adj`)
